...
dw_Util.isTouchDevice = function() { return 'ontouchend' in document; };
...
var dw_Tooltip = {
    ...
    init: function() {
        ...
        if (_this.hasSupport()) {
            ...
            //ECG:Modifico zindex para integracion con modalpopups de ACT
    ...
    getActuatorInfo: function(tgt) {
        //ECG:Exit early if tooltip of our custome type
        if (dw_Tooltip.content_source == 'TooltipText') {
            if ((tgt.attributes && tgt.attributes['tooltiptext'])) {
                return {
                    msg: tgt.attributes['tooltiptext'].value,
                    id: ''
                };
            } else {
                //dw_Tooltip.deactivate();
                dw_Tooltip.hide();
            }
        }
